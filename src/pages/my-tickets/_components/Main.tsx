...
const Main = () => {
	...
	const confirmTransfer = useCallback(() => {
		...
		fetch(`${API_HOST}/v1/mytickets/transfers`, {
			...
			body: JSON.stringify({
				...
				tickets: tickets.filter(ticket => selectedTransferTickets.includes(ticket.id))
					.map(ticket => ({
						id: ticket.id,
	...
	const handleTransferRecipientNameChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
		setRecipientName(e.target.value);
	}, [setRecipientName]);

	const handleTransferRecipientEmailChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
		setRecipientEmail(e.target.value);
	}, [setRecipientEmail]);

	const handleTicketUpgradeCheckboxChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
	...
	const handleTicketTransferCheckboxChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
		const { value } = e.target;
	...
	return (
		<main className={styles.main}>
						...
						{Object.values(eventsById).map(({ name: eventName, dateString, timeString, tickets: eventTickets }) => (
							<div className="event-group" key={eventName}>
								...
								<div className={styles.upgradeTransfer}>
									{eventTickets.some(ticket => ticket.upgradeProductId) && <button className="yellow" onClick={openUpgradeDialog}>Upgrade</button>}
									<button className="yellow" onClick={openTransferDialog}>Transfer</button>
								</div>
								<ul>
								{[...eventTickets]
									.sort((a, b) =>  {
			...
			<dialog className="upgrade-dialog" ref={upgradeDialog}>
				...
				<ul>
					{tickets.filter(ticket => ticket.upgradeProductId).map(({ id, eventName, upgradePrice }, i) => (
						<li key={id}>
							<input type="checkbox"
								value={id}
							...
							/>
							<label htmlFor={`ticket-upgrade-${id}-checkbox`}>#{i + 1} - {eventName} - ${upgradePrice}</label>
						</li>
					))}
				</ul>
				<div className={styles.dialogCta}>
					<button className="yellow" onClick={closeUpgradeDialog}>Upgrade</button>
					<button className="yellow" onClick={closeUpgradeDialog}>Cancel</button>
				</div>
			</dialog>
			<dialog className={classNames(styles.dialog, styles.transferDialog)} ref={transferDialog}>
				<h4>Transfer Tickets</h4>
				<p>Please select the tickets you would like to transfer and enter recipient's name and email. Once a ticket has been transferred, it cannot be taken back! Only transfer to people you trust.</p>
				<ul>
					{tickets.map(({ id, eventName }, i) => (
						<li key={id}>
							...
							/>
							<label htmlFor={`ticket-transfer-${id}-checkbox`}>#{i + 1} - {eventName}</label>
						</li>
					))}
				</ul>
				<div>
					...
					<input type="text" id="recipient-name" className={classNames({[styles.inputError as string]: recipientNameError})} onChange={handleTransferRecipientNameChange} value={recipientName} />
				</div>
				<div>
					...
					<input type="email" id="recipient-email" className={classNames({[styles.inputError as string]: recipientEmailError})} onChange={handleTransferRecipientEmailChange} value={recipientEmail} pattern="^.+@.+\..{2,}$" required />
				</div>
				<div className={styles.dialogCta}>
					<button className="yellow" onClick={confirmTransfer}>Transfer</button>
					<button className="yellow" onClick={closeTransferDialog}>Cancel</button>
				</div>
			</dialog>
			<dialog className={classNames(styles.dialog, styles.transferSuccessDialog)} ref={transferSuccessDialog}>
				<h4>Success!</h4>
				<p>Your tickets have been successfully transferred to <strong>{recipientName}</strong> and they have been emailed their confirmation to <strong>{recipientEmail}</strong>.</p>
				<div className={styles.dialogCta}>
					<button className="yellow" onClick={closeTransferSuccessDialog}>Ok</button>
				</div>
			</dialog>
		</main>
	);
